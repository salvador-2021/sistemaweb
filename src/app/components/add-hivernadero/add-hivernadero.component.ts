...
@Component({
  ...
  styleUrls: ['./add-hivernadero.component.css'],
  providers: [HivernaderoService]
})
export class AddHivernaderoComponent implements OnInit {
  @ViewChild("contenedorImg") contenedorImg: ElementRef;

  private dataModel: HivernaderoModel;
  public validacionForm: FormGroup;

  private dataModelUpdate: HivernaderoModel;
  ...
  constructor(
    ...
    this.validacionForm = this.formBuilder.group({
      ...
      instru_cuidado: ['', [Validators.required, Validators.maxLength(100)]],
  ...
  uploadImage() {
    ...
    if (this.listImagen.length < 3) {
      ...
      this._hivernaderoService.uploadImage(this.currentFileUpload, this._idProducto).subscribe(
        event => {
  ...
  getImageName(nameImage) {
    ...
    this._hivernaderoService.getImageName(nameImage).subscribe(
      response => {
  ...
  deleteImage(nameImage) {
    this._hivernaderoService.deleteImageProduct(nameImage).subscribe(
      response => {
  ...
  showNumber(nombreCampo, valor) {
    ...
    if (nombreCampo == "instru_cuidado") { this.listaDatosMostrar.instru_cuidado = valor; }
