...
@Component({
  ...
  styleUrls: ['./add-fotografia.component.css'],
...
export class AddFotografiaComponent implements OnInit {
  ...
  private dataModelUpdate: FotografiaModel;
  ...
  constructor(
    ...
    this.dataModel = new FotografiaModel("", "", "", "", "", "", 0, 0, null, null, 0, null, null);
  ...
  onSubmit() {
    ...
    if (this.campaignOne.value.start == null || this.campaignOne.value.end == null) {
      ...
      this._fotografiaService.saveData(this.dataModel).subscribe(
        response => {
          if (response.status == 'success') {
            this.ngxLoaderService.stop(); // FINALIZA EL EFECTO DE CARGA
            ...
            Swal.fire("Producto creado",
              ...
              "success").then((value) => {
                ...
                this._router.navigate(['/negocio/agregar-fotografia', this._idProducto]);
 ...
 */
  onSubmitEdit() {
    ...
    if (this.campaignOne.value.start == null || this.campaignOne.value.end == null) {
      ...
      this._fotografiaService.updateProductNegocio(this._idProducto, this.dataModel).subscribe(
        response => {
  ...
  uploadImage() {
    ...
    if (this.listImagen.length < 3) {
      ...
      this._fotografiaService.uploadImage(this.currentFileUpload, this._idProducto).subscribe(
        event => {
  ...
  getImageName(nameImage) {
    ...
    this._fotografiaService.getImageName(nameImage).subscribe(
      response => {
  ...
  deleteImage(nameImage) {
    this._fotografiaService.deleteImageProduct(nameImage).subscribe(
      response => {
