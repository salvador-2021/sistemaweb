...
@Component({
  selector: 'app-add-joyeria',
  templateUrl: './add-joyeria.component.html',
  styleUrls: ['./add-joyeria.component.css'],
  providers: [JoyeriaService]
})
export class AddJoyeriaComponent implements OnInit {
  public _datosGlobales: DatosGlobales;
  @ViewChild("contenedorImg") contenedorImg: ElementRef;

  private dataModel: JoyeriaModel;
  public validacionForm: FormGroup;

  private dataModelUpdate: JoyeriaModel;
        ...
        this._joyeriaService.getProductNegocio(_id).subscribe(
          ...
          response => {
            ...
            if (response.status == 'success') {
              ...
              this.dataModelUpdate = response.message.joyeria;
              ...
              this.validacionForm.setValue(
                {
                  ...
                  kilataje: this.dataModelUpdate[0].kilataje,
          ...
          if (response.status == 'success') {
            ...
            Swal.fire("Producto creado",
              ...
              "success").then((value) => {
                ...
                this._router.navigate(['/negocio/agregar-joyeria', this._idProducto]);
              });
          }
        },
        error => {
  ...
  recogerAsignar() {
    ...
    this.dataModel.kilataje = this.validacionForm.value.kilataje;
  ...
  */
  ...
  uploadImage() {
    ...
    if (this.listImagen.length < 3) {
      ...
      this._joyeriaService.uploadImage(this.currentFileUpload, this._idProducto).subscribe(
        event => {
  ...
  getImageName(nameImage) {
    ...
    this._joyeriaService.getImageName(nameImage).subscribe(
      response => {
  ...
  deleteImage(nameImage) {
    this._joyeriaService.deleteImageProduct(nameImage).subscribe(
      response => {
  ...
  showNumber(nombreCampo, valor) {
    ...
    if (nombreCampo == "kilataje") { this.listaDatosMostrar.kilataje = valor; }
