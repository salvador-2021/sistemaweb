...
@Component({
  ...
  templateUrl: './add-carpinteria.component.html',
  styleUrls: ['./add-carpinteria.component.css'],
  providers: [CarpinteriaService]
})
export class AddCarpinteriaComponent implements OnInit {

  @ViewChild("contenedorImg") contenedorImg: ElementRef;

  private dataModel: CarpinteriaModel;
  public validacionForm: FormGroup;

  private dataModelUpdate: CarpinteriaModel;
          ...
          if (response.status == 'success') {
            ...
            Swal.fire("Producto creado",
              ...
              "success").then((value) => {
                ...
                this._router.navigate(['/negocio/agregar-carpinteria', this._idProducto]);
 ...
 */
  ...
  uploadImage() {
    ...
    if (this.listImagen.length < 3) {
      ...
      this._carpinteriaService.uploadImage(this.currentFileUpload, this._idProducto).subscribe(
        event => {
  ...
  getImageName(nameImage) {
    ...
    this._carpinteriaService.getImageName(nameImage).subscribe(
      response => {
