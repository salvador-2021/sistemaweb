...
export class TblPlomeriaComponent {
  ...
  displayedColumns: string[] = ['nombre', 'marca', 'unidadventa', 'garantia', 'precio_anterior', 'precio', 'existencia', 'fecha_promocion', 'acciones'];
  dataSource: MatTableDataSource<PlomeriaModel>;
  ...
  */
  ...
  deleteAllProduct(){
    this.ngxLoaderService.start(); // INICIA EL EFECTO DE CARGA
    this._plomeriaService.deleteAllImageProduct().subscribe(
      response=>{
        if(response.status =="success"){
          this.ngxLoaderService.stop(); // FINALIZA EL EFECTO DE CARGA
