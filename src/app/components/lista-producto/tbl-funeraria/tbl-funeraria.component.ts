...
@Component({
  ...
  styleUrls: ['./tbl-funeraria.component.css'],
  providers: [FunerariaService]
})
export class TblFunerariaComponent {
  public products: FunerariaModel[];
  public title: String;
  ...
  displayedColumns: string[] = ['nombre', 'unidadventa', 'medidas', 'persona', 'precio_anterior', 'precio', 'existencia', 'fecha_promocion', 'acciones'];
  dataSource: MatTableDataSource<FunerariaModel>;
    ...
    this._funerariaService.getProductNegocio(_id).subscribe(
      response => {
        ...
        if (response.status == 'success') {
          ...
          let listImagen = response.message.funeraria[0].imagen;
          //recorremos la lista de nombre de las imagenes
          if (listImagen != null) {
            listImagen.forEach(data => {
              this._funerariaService.deleteImageProduct(data.ruta).subscribe(
 ...
 */
  ...
  listaProductosNegocio(estado) {
    ...
    this._funerariaService.getListProductNegocio(estado).subscribe(
      response => {
  ...
  deleteAllProduct(){
    this.ngxLoaderService.start(); // INICIA EL EFECTO DE CARGA
    this._funerariaService.deleteAllImageProduct().subscribe(
      response=>{
        if(response.status =="success"){
          this.ngxLoaderService.stop(); // FINALIZA EL EFECTO DE CARGA
          this.listaProductosNegocio(1);
