...
export class TblFierroComponent {
  ...
  displayedColumns: string[] = ['nombre', 'unidadventa', 'medidas', 'color', 'precio_anterior', 'precio', 'existencia', 'fecha_promocion', 'acciones'];
  dataSource: MatTableDataSource<FierroModel>;
  ...
  */
  ...
  deleteAllProduct(){
    this.ngxLoaderService.start(); // INICIA EL EFECTO DE CARGA
    this._fierroService.deleteAllImageProduct().subscribe(
      response=>{
        if(response.status =="success"){
          this.ngxLoaderService.stop(); // FINALIZA EL EFECTO DE CARGA
          this.listaProductosNegocio(1);
