...
export class TblMicroondaComponent {
  ...
  displayedColumns: string[] = ['nombre', 'voltaje', 'potencia', 'panel_control', 'precio_anterior', 'precio', 'existencia', 'fecha_promocion', 'acciones'];
  dataSource: MatTableDataSource<MicroondaModel>;
  ...
  */
  ...
  deleteAllProduct(){
    this.ngxLoaderService.start(); // INICIA EL EFECTO DE CARGA
    this._microondaService.deleteAllImageProduct().subscribe(
      response=>{
        if(response.status =="success"){
          this.ngxLoaderService.stop(); // FINALIZA EL EFECTO DE CARGA
