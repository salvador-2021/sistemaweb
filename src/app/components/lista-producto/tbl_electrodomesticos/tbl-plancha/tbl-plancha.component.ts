...
@Component({
  selector: 'app-tbl-plancha',
  templateUrl: './tbl-plancha.component.html',
  styleUrls: ['./tbl-plancha.component.css'],
  providers: [PlanchaService]
})
export class TblPlanchaComponent {

  public products: PlanchaModel[];
  public title: String;
  ...
  displayedColumns: string[] = ['nombre', 'marca', 'potencia', 'medidas', 'precio_anterior', 'precio', 'existencia', 'fecha_promocion', 'acciones'];
  dataSource: MatTableDataSource<PlanchaModel>;
 ...
 */
    ...
    this._planchaService.getProductNegocio(_id).subscribe(
      response => {
        ...
        if (response.status == 'success') {
          ...
          let listImagen = response.message.plancha[0].imagen;
          //recorremos la lista de nombre de las imagenes
          if (listImagen != null) {
            listImagen.forEach(data => {
              this._planchaService.deleteImageProduct(data.ruta).subscribe(
                response => { 
  ...
  deleteData(_id) {
    ...
    this._planchaService.deleteProductNegocio(_id).subscribe(
      response => {
  ...
  listaProductosNegocio(estado) {
    ...
    this._planchaService.getListProductNegocio(estado).subscribe(
      response => {
        this.ngxLoaderService.stop(); // FINALIZA EL EFECTO DE CARGA
  ...
  updateStatusProducto(_id, estado) {
    ...
    this._planchaService.updateStatusProduct(_id, estadoEnviar).subscribe(
      response => {
        this.ngxLoaderService.stop(); // FINALIZA EL EFECTO DE CARGA
  ...
  deleteAllProduct(){
    this.ngxLoaderService.start(); // INICIA EL EFECTO DE CARGA
    this._planchaService.deleteAllImageProduct().subscribe(
      response=>{
        this.ngxLoaderService.stop(); // FINALIZA EL EFECTO DE CARGA
