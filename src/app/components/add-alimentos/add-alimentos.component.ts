...
export class AddAlimentosComponent implements OnInit {
  ...
  private dataModel: AlimentoModel;
  public validacionForm: FormGroup;

  private dataModelUpdate: AlimentoModel;
  ...
  uploadImage() {
    if (this.listImagen.length < 3) {
      ...
      this._alimentoService.uploadImage(this.currentFileUpload, this._idProducto).subscribe(
        event => {
  ...
  getImageName(nameImage) {
    ...
    this._alimentoService.getImageName(nameImage).subscribe(
      response => {
  ...
  deleteImageMongodb(nameImage) {
    ...
    //segundo parametro =>cantidad de datos a eliminar comenzando desde la posicion indicada
