...
@Component({
  ...
  styleUrls: ['./add-herreria.component.css'],
...
export class AddHerreriaComponent implements OnInit {
  ...
  private dataModelUpdate: HerreriaModel;
  ...
  uploadImage() {
    ...
    if (this.listImagen.length < 3) {
      ...
      this._herreriaService.uploadImage(this.currentFileUpload, this._idProducto).subscribe(
        event => {
  ...
  getImageName(nameImage) {
    ...
    this._herreriaService.getImageName(nameImage).subscribe(
      response => {
  ...
  deleteImage(nameImage) {
    this._herreriaService.deleteImageProduct(nameImage).subscribe(
      response => {
