...
@Component({
  ...
  styleUrls: ['./add-plomeria.component.css'],
...
export class AddPlomeriaComponent implements OnInit {
  ...
  private dataModelUpdate: PlomeriaModel;
  ...
  constructor(
    ...
    this.dataModel = new PlomeriaModel("", "", "", "", "", "", "", 0, 0, null, null);
        ...
        if (response.status == 'success') {
          ...
          Swal.fire("Producto creado",
            ...
            "success").then((value) => {
              ...
              this._router.navigate(['/add-plomeria', this._idProducto]);
            });
        }
      },
      error => {
  ...
  uploadImage() {
    ...
    if (this.listImagen.length < 3) {
      ...
      this._plomeriaService.uploadImage(this.currentFileUpload, this._idProducto).subscribe(
        event => {
  ...
  getImageName(nameImage) {
    ...
    this._plomeriaService.getImageName(nameImage).subscribe(
      response => {
  ...
  deleteImage(nameImage) {
    this._plomeriaService.deleteImageProduct(nameImage).subscribe(
      response => {
